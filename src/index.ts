...
const ICH_GUIDELINES: Record<string, { title: string; scope: string; keyRequirements: string[]; url: string }> = {
  ...
  "M7(R2)": {
    ...
    keyRequirements: [
      "TTC (Threshold of Toxicological Concern) approach: 1.5 μg/day default",
  ...
  "M9": {
    ...
    keyRequirements: [
      ...
      "High permeability: ≥85% absorbed in humans",
      "Rapid dissolution: ≥85% dissolved in 30 minutes",
  ...
  "S1B(R1)": {
    ...
    keyRequirements: [
      "Required for drugs intended for continuous use >6 months",
  ...
  "Q1A(R2)": {
    ...
    keyRequirements: [
      "Long-term: 25°C/60%RH for 12 months minimum at submission",
      ...
      "Shelf life based on real-time data; extrapolation limited to 1.5× available data"
    ],
  ...
  tools: [
    {
      ...
      description: "Look up an ICH guideline by code (e.g. E6(R3), M4, Q1A). Returns scope, key requirements, and official URL. Essential for regulatory writers and submission teams verifying compliance requirements.",
      inputSchema: {
        type: "object",
        properties: {
          guideline_code: {
            type: "string",
            description: "ICH guideline code, e.g. 'E6(R3)', 'M4(R4)', 'Q3D(R2)', 'M7(R2)'"
          }
        },
    ...
    },
    {
      ...
      description: "Map a document type or data package to the correct CTD/eCTD module and section. Returns the full section hierarchy and what belongs there. Critical for building compliant NDA, BLA, or MAA submissions.",
      inputSchema: {
        type: "object",
        properties: {
          module: {
            type: "string",
            description: "CTD module number: '1', '2', '3', '4', or '5'"
          },
          submission_type: {
            type: "string",
            description: "Submission type: 'NDA', 'BLA', 'MAA', or 'JNDA'"
          }
        },
        required: ["module"]
      }
    },
    {
      ...
      description: "Check a list of provided CTD sections against required sections for a given submission type, and identify gaps. Returns missing sections and recommendations.",
      inputSchema: {
        type: "object",
        properties: {
          provided_sections: {
            type: "array",
            items: { type: "string" },
            description: "List of CTD sections already prepared, e.g. ['3.2.S.1', '3.2.S.2', '3.2.P.1']"
          },
          submission_type: {
            type: "string",
            description: "Target submission type: 'NDA', 'BLA', or 'MAA'"
          },
          module: {
            type: "string",
            description: "Which module to check completeness for: '3', '4', or '5'"
          }
        },
    ...
    },
    {
      ...
      description: "Retrieve common deficiency areas and typical agency questions for FDA or EMA submissions by domain (CMC, Clinical, Labelling, Pharmacovigilance). Helps teams anticipate and proactively address reviewer concerns.",
      inputSchema: {
        type: "object",
        properties: {
          agency: {
            type: "string",
            description: "Regulatory agency: 'FDA' or 'EMA'"
          },
          domain: {
            type: "string",
            description: "Area of concern: 'CMC', 'Clinical', 'Labelling', or 'Pharmacovigilance'"
          }
        },
    ...
    },
    {
      ...
      description: "Generate a submission readiness checklist for a given submission type and agency. Returns ordered list of required documents, data packages, and administrative requirements.",
      inputSchema: {
        type: "object",
        properties: {
          submission_type: {
            type: "string",
            description: "Type: 'NDA', 'BLA', 'MAA', 'IND', 'CTA', 'IMPD'"
          },
          agency: {
            type: "string",
            description: "Target agency: 'FDA', 'EMA', 'PMDA', 'Health Canada'"
          }
        },
    ...
    },
    {
      ...
      description: "Given a description of a study, process, or document, check it against the relevant ICH guideline requirements and return a compliance assessment with pass/fail items and recommendations.",
      inputSchema: {
        type: "object",
        properties: {
          description: {
            type: "string",
            description: "Description of the study, process, or document to assess"
          },
          guideline_code: {
            type: "string",
            description: "ICH guideline to check against, e.g. 'E6(R3)', 'Q1A(R2)', 'M7(R2)'"
          }
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "lookup_ich_guideline") {
    const schema = z.object({ guideline_code: z.string() });
    const { guideline_code } = schema.parse(args);
    const code = guideline_code.toUpperCase().trim();
    const guideline = ICH_GUIDELINES[code];

    if (!guideline) {
      const available = Object.keys(ICH_GUIDELINES).join(", ");
      return {
        content: [{
          type: "text",
          text: `Guideline "${code}" not found in database. Available guidelines: ${available}\n\nFor comprehensive ICH guideline access, visit: https://www.ich.org/page/ich-guidelines`
        }]
      };
    }
    ...
    return {
      content: [{
        type: "text",
        text: `# ICH ${code}: ${guideline.title}\n\n**Scope:** ${guideline.scope}\n\n**Key Requirements:**\n${guideline.keyRequirements.map((r, i) => `${i + 1}. ${r}`).join("\n")}\n\n**Official Resource:** ${guideline.url}`
      }]
    };
  }

  if (name === "map_ctd_section") {
    const schema = z.object({ module: z.string(), submission_type: z.string().optional() });
    const { module, submission_type } = schema.parse(args);
    ...
    if (!moduleData) {
      return {
        content: [{
          type: "text",
          text: `Module "${module}" not recognised. Valid CTD modules are 1, 2, 3, 4, and 5.`
        }]
      };
    }
  ...
  if (name === "check_ctd_completeness") {
    const schema = z.object({
      provided_sections: z.array(z.string()),
      submission_type: z.string(),
      module: z.string()
    });
    const { provided_sections, submission_type, module } = schema.parse(args);
    const moduleData = CTD_MODULES[module];
    ...
    return {
      content: [{
        type: "text",
        text: `# CTD Module ${module} Completeness Check\n**Submission Type:** ${submission_type}\n**Completeness Score:** ${score}%\n\n**✅ Provided (${provided.length}):**\n${provided.map(s => `- ${s}: ${moduleData.sections[s]}`).join("\n")}\n\n**❌ Missing (${missing.length}):**\n${missing.map(s => `- ${s}: ${moduleData.sections[s]}`).join("\n")}\n\n${score === 100 ? "Module appears complete." : `Action required: ${missing.length} section(s) need to be addressed before submission.`}`
      }]
    };
  }

  if (name === "get_agency_deficiency_guidance") {
    const schema = z.object({ agency: z.string(), domain: z.string() });
    const { agency, domain } = schema.parse(args);
    const agencyKey = agency.toUpperCase() as keyof typeof AGENCY_DEFICIENCIES;
    const agencyData = AGENCY_DEFICIENCIES[agencyKey];

    if (!agencyData) {
      return { content: [{ type: "text", text: `Agency "${agency}" not found. Supported: FDA, EMA.` }] };
    }

    const domainKey = domain as keyof typeof agencyData;
    const deficiencies = (agencyData as Record<string, string[]>)[domain];

    if (!deficiencies) {
      const available = Object.keys(agencyData).join(", ");
      return { content: [{ type: "text", text: `Domain "${domain}" not found for ${agency}. Available: ${available}` }] };
    }

    return {
      content: [{
        type: "text",
        text: `# ${agency} Common Deficiencies — ${domain}\n\nThese are frequently raised issues in ${agency} ${domain} reviews. Address proactively in your submission:\n\n${deficiencies.map((d, i) => `${i + 1}. ${d}`).join("\n")}`
      }]
    };
  }

  if (name === "generate_submission_checklist") {
    const schema = z.object({ submission_type: z.string(), agency: z.string() });
    const { submission_type, agency } = schema.parse(args);

    const checklists: Record<string, Record<string, string[]>> = {
      NDA: {
        FDA: [
          "Cover letter and Form FDA 356h",
          "Module 1: Proposed labelling (PI, Medication Guide if applicable)",
          "Module 1: Patent certifications (Paragraph IV if applicable)",
          "Module 1: Debarment certification",
          "Module 1: Financial disclosure (Form FDA 3454/3455)",
          "Module 2.3: Quality Overall Summary",
          "Module 2.4: Nonclinical Overview",
          "Module 2.5: Clinical Overview",
          "Module 2.6: Nonclinical Written and Tabulated Summaries",
          "Module 2.7: Clinical Summary",
          "Module 3: Full CMC documentation (3.2.S and 3.2.P)",
          "Module 4: All nonclinical study reports",
          "Module 5: All clinical study reports (pivotal trials)",
          "Module 5: Integrated Summary of Safety (ISS)",
          "Module 5: Integrated Summary of Efficacy (ISE)",
          "PREA compliance / iPSP or waiver/deferral",
          "REMS (if required based on safety profile)",
          "User fee payment confirmation (PDUFA)"
        ]
      },
      MAA: {
        EMA: [
          ...
          "Module 1.3: Product information (SmPC, PIL, labelling)",
          "Module 1.7: EudraVigilance registration confirmation",
          "Module 1.8: Information relating to clinical trials (EudraCT)",
          "Module 1.10: Pharmacovigilance system summary",
          "Module 1.11: Risk Management Plan (RMP)",
          "Module 2–5: Full CTD technical documentation",
          "Paediatric Investigation Plan (PIP) compliance",
          "CHMP scientific advice follow-up (if applicable)",
          "GMP compliance documentation for manufacturing sites",
          "ASMF or CEP for drug substance (if applicable)",
          "Environmental Risk Assessment (ERA)",
          "Biosimilar comparability data (for biosimilars only)",
          "EMA application fee payment"
        ]
      },
      IND: {
        FDA: [
          ...
          "Protocol(s) and amendments",
          "Chemistry, Manufacturing, and Controls (CMC) information",
          "Pharmacology and toxicology information",
          "Previous human experience (if any)",
          "Additional information (special topics: PK, bioavailability)",
          "Institutional Review Board (IRB) confirmation",
          "Sponsor-investigator certifications"
        ]
      }
    };

    const submissionKey = submission_type.toUpperCase();
    const agencyKey = agency.toUpperCase();

    const checklist = checklists[submissionKey]?.[agencyKey];

    if (!checklist) {
      return {
        content: [{
          type: "text",
          text: `Checklist for ${submission_type} (${agency}) not yet in database. Currently available: NDA (FDA), MAA (EMA), IND (FDA). Contact support for additional submission types.`
        }]
      };
    }
    ...
    return {
      content: [{
        type: "text",
        text: `# ${submission_type} Submission Checklist (${agency})\n\n${checklist.map((item, i) => `- [ ] ${i + 1}. ${item}`).join("\n")}\n\n**Total items:** ${checklist.length}\n\n> This checklist reflects standard requirements. Always verify against the latest ${agency} guidance documents and any product-specific requirements.`
      }]
    };
  }

  if (name === "check_ich_compliance") {
    const schema = z.object({ description: z.string(), guideline_code: z.string() });
    const { description, guideline_code } = schema.parse(args);
    const code = guideline_code.toUpperCase().trim();
    const guideline = ICH_GUIDELINES[code];

    if (!guideline) {
      return {
        content: [{ type: "text", text: `Guideline ${code} not in database. Available: ${Object.keys(ICH_GUIDELINES).join(", ")}` }]
      };
    }
    ...
    const assessments = guideline.keyRequirements.map(req => {
      ...
      return { requirement: req, status: mentioned ? "✅ Addressed" : "⚠️ Not clearly addressed — review needed" };
    });

    const passed = assessments.filter(a => a.status.startsWith("✅")).length;
    const total = assessments.length;

    return {
      content: [{
        type: "text",
        text: `# ICH ${code} Compliance Assessment\n\n**Guideline:** ${guideline.title}\n**Score:** ${passed}/${total} requirements addressed\n\n${assessments.map(a => `**${a.status}**\n_Requirement:_ ${a.requirement}`).join("\n\n")}\n\n> **Note:** This is a keyword-based preliminary assessment. Full compliance requires expert review against the complete guideline text at ${guideline.url}`
      }]
    };
  }
...
async function main() {
  ...
  console.error("Regulatory Submission Intelligence MCP server running on stdio");
}
